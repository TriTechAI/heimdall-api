...
#### **2.1: 标签管理系统 (优化后：P1级，6小时)**
- [x] `(#T120)` [P1][common] **标签数据模型与DAO** *(120分钟)* ✅ **DONE** - *2024-07-07*
  - [x] `common/model/tag.go`: 定义独立的Tag数据模型
    - [x] 基础字段：ID, Name, Slug, Description, Color, FeaturedImage
    - [x] 统计字段：PostCount, CreatedAt, UpdatedAt  
    - [x] SEO字段：MetaTitle, MetaDescription
    - [x] 状态字段：Visibility (public/internal)
  - [x] `common/dao/tagdao.go`: 实现TagDAO完整接口
    - [x] 基础CRUD：Create, GetByID, GetBySlug, Update, Delete (软删除)
    - [x] 查询方法：List, GetPublishedList, GetPopularTags
    - [x] 统计方法：IncrementPostCount, UpdatePostCount
    - [x] 索引管理：CreateIndexes, buildQuery, buildSort
  - [x] `common/constants/tag.go`: 标签常量定义和验证工具
  - [x] 完整的单元测试覆盖 (574行测试代码，379行DAO测试)
  - **依赖**: T012 ✅
  - **验收**: ✅ 标签模型完整，DAO功能齐全，企业级代码质量，测试覆盖全面

- [x] `(#T121)` [P1][admin-api] **标签管理API** *(120分钟)* ✅ **DONE** - *2024-07-07*
  - [x] 在`admin.api`中定义标签管理接口
    - [x] GET /api/v1/admin/tags (列表，支持搜索过滤、分页、排序)
    - [x] POST /api/v1/admin/tags (创建)
    - [x] GET /api/v1/admin/tags/{id} (详情)
    - [x] PUT /api/v1/admin/tags/{id} (更新)
    - [x] DELETE /api/v1/admin/tags/{id} (删除)
  - [x] 实现对应的Logic层完整业务逻辑
    - [x] CreateTagLogic: 标签创建，包含slug生成和验证、输入验证
    - [x] GetTagListLogic: 标签列表，支持多维度过滤、分页、排序
    - [x] GetTagDetailLogic: 标签详情获取，包含权限检查
    - [x] UpdateTagLogic: 标签更新，包含slug冲突检查、部分更新
    - [x] DeleteTagLogic: 标签删除，包含文章关联检查，防止删除有关联的标签
    - [x] 完整的权限检查和输入验证机制
    - [x] 企业级错误处理和日志记录
  - [x] API类型定义：TagDetailInfo, TagListRequest/Response等完整类型系统
  - [x] goctl代码生成成功，项目编译通过
  - **依赖**: T120 ✅
...
- [ ] `(#T103)` [P1][admin-api] **安全中间件** *(120分钟)*
  - [ ] 实现JWT黑名单检查中间件
    - [ ] 集成common/cache/jwt_blacklist.go
    - [ ] 自动检查Token是否被撤销
  - [ ] 实现IP限流中间件
    - [ ] 集成common/cache/rate_limiter.go
    - [ ] 支持不同端点的差异化限流
  - [ ] 实现操作审计中间件
    - [ ] 记录关键操作日志
    - [ ] 异步写入，避免性能影响
...
- [ ] `(#T131)` [P2][admin-api] **评论管理API** *(120分钟)*
  - [ ] 评论管理接口（批量操作优化）
    - [ ] GET /api/v1/admin/comments (列表，多维度过滤)
    - [ ] PUT /api/v1/admin/comments/{id}/approve (审核通过)
    - [ ] PUT /api/v1/admin/comments/{id}/reject (审核拒绝)
    - [ ] POST /api/v1/admin/comments/batch (批量操作)
    - [ ] DELETE /api/v1/admin/comments/{id} (删除)
  - [ ] 实现高效的批量审核逻辑
  - **依赖**: T130 ✅
  - **验收**: 评论管理高效，支持批量操作

- [ ] `(#T132)` [P2][public-api] **公开评论功能** *(90分钟)*
  - [ ] 评论展示和提交接口
    - [ ] GET /api/v1/public/posts/{postId}/comments (获取评论)
    - [ ] POST /api/v1/public/posts/{postId}/comments (提交评论)
  - [ ] 实现评论防垃圾机制
    - [ ] 内容过滤和频率限制
    - [ ] 自动垃圾检测
  - **依赖**: T130 ✅
...
#### **3.1: 内容发现与搜索 (优化后：P2级，5小时)**
- [ ] `(#T160)` [P2][public-api] **内容发现API** *(120分钟)*
  - [ ] 实现热门内容接口
    - [ ] GET /api/v1/public/posts/popular (热门文章)
    - [ ] GET /api/v1/public/posts/recent (最新文章)
    - [ ] GET /api/v1/public/archives (归档列表)
  - [ ] 实现作者信息接口
    - [ ] GET /api/v1/public/authors (作者列表)
    - [ ] GET /api/v1/public/authors/{username} (作者详情)
  - [ ] 支持缓存和性能优化
  - **依赖**: T031 ✅ (文章DAO)
  - **验收**: 内容发现功能丰富，响应快速

- [ ] `(#T161)` [P2][public-api] **搜索功能** *(120分钟)*
  - [ ] 实现全文搜索接口
    - [ ] GET /api/v1/public/search (搜索文章和页面)
    - [ ] 支持标题、内容、标签搜索
    - [ ] 支持搜索结果高亮和排序
  - [ ] 实现搜索优化
    - [ ] 搜索关键词缓存
...
- [ ] `(#T162)` [P2][public-api] **SEO与内容优化** *(60分钟)*
  - [ ] 实现SEO支持接口
...
#### **3.2: 站点配置管理 (P2级，4小时)**
- [ ] `(#T200)` [P2][common] **配置数据模型** *(90分钟)*
  - [ ] `common/model/setting.go`: 站点设置模型
    - [ ] 基础配置：SiteName, Description, Keywords
    - [ ] 主题配置：Theme, Logo, Favicon
    - [ ] 社交配置：SocialLinks, Contact
  - [ ] `common/dao/settingdao.go`: 配置DAO
    - [ ] 支持配置的读取、更新、缓存
    - [ ] 配置版本管理和回滚
  - **依赖**: T012 ✅
  - **验收**: 配置管理灵活，支持动态更新

- [ ] `(#T201)` [P2][admin-api] **站点配置API** *(90分钟)*
  - [ ] 实现配置管理接口
    - [ ] GET /api/v1/admin/settings (获取配置)
    - [ ] PUT /api/v1/admin/settings (更新配置)
    - [ ] POST /api/v1/admin/settings/reset (重置配置)
  - [ ] 实现配置验证和安全检查
  - **依赖**: T200 ✅
  - **验收**: 配置管理安全，验证完整

- [ ] `(#T202)` [P2][public-api] **公开配置接口** *(60分钟)*
  - [ ] 实现公开配置获取
    - [ ] GET /api/v1/public/site/info (站点信息)
    - [ ] GET /api/v1/public/navigation (导航菜单)
  - [ ] 支持配置缓存和CDN
  - **依赖**: T201 ✅
  - **验收**: 公开配置访问快速，信息准确

#### **3.3: 媒体文件管理 (P2级，4小时)**
- [ ] `(#T210)` [P2][common] **文件存储抽象** *(90分钟)*
  - [ ] `common/client/storage.go`: 存储接口定义
    - [ ] 支持本地存储和云存储(未来扩展)
    - [ ] 文件上传、删除、URL生成
  - [ ] `common/model/media.go`: 媒体文件模型
    - [ ] 文件信息：Name, Size, Type, URL
    - [ ] 元数据：Author, CreatedAt, Tags
  - **依赖**: T012 ✅
  - **验收**: 存储抽象设计合理，易于扩展

- [ ] `(#T211)` [P2][admin-api] **媒体管理API** *(150分钟)*
  - [ ] 实现文件管理接口
    - [ ] POST /api/v1/admin/media/upload (文件上传)
    - [ ] GET /api/v1/admin/media (文件列表)
    - [ ] DELETE /api/v1/admin/media/{id} (删除文件)
  - [ ] 实现安全检查
    - [ ] 文件类型验证
    - [ ] 文件大小限制
    - [ ] 恶意文件检测
...
#### **4.1: 数据统计与分析 (P2级，4小时)**
- [ ] `(#T300)` [P2][admin-api] **Dashboard统计** *(120分钟)*
  - [ ] 实现统计数据API
    - [ ] GET /api/v1/admin/dashboard/stats (概览统计)
    - [ ] GET /api/v1/admin/analytics/posts (文章统计)
    - [ ] GET /api/v1/admin/analytics/users (用户统计)
  - [ ] 实现趋势分析
    - [ ] 文章发布趋势
    - [ ] 用户注册趋势
    - [ ] 访问量趋势分析
  - **依赖**: T031 ✅ (文章DAO), T021 ✅ (用户DAO)
  - **验收**: 统计数据准确，图表展示清晰

- [ ] `(#T301)` [P2][common] **访问统计模型** *(120分钟)*
  - [ ] `common/model/analytics.go`: 统计数据模型
    - [ ] 页面访问统计
    - [ ] 用户行为跟踪
    - [ ] 搜索关键词统计
  - [ ] `common/dao/analyticsdao.go`: 统计数据DAO
    - [ ] 高性能数据聚合
    - [ ] 时间维度统计
  - **依赖**: T300 ✅
  - **验收**: 统计模型高效，数据聚合快速
...
- [ ] `(#T321)` [P2][common] **监控指标** *(90分钟)*
  - [ ] `common/metrics/prometheus.go`: Prometheus指标
    ...
    - [ ] 错误率和QPS监控
    - [ ] 数据库连接池监控
  - [ ] `common/metrics/health.go`: 健康检查
    - [ ] 服务健康状态检查
    - [ ] 依赖服务检查
  - **依赖**: T320 ✅
  - **验收**: 监控指标覆盖关键业务点

- [ ] `(#T322)` [P2][Both] **运维接口** *(90分钟)*
  - [ ] 实现运维管理接口
    - [ ] GET /health (健康检查)
